...
// Simple exercise from online tutorial

void limit_simple_counting( int n_obs = 3,           // number of observed events
                            ...
                            double sigmab = 0.2 )   // relative uncertainty in b
{
  ...
  w.factory("sum:nexp(s[1,0,5],b[1,0,10])");
  // Poisson of (n | s+b)
  w.factory("Poisson:pdf(nobs[0,30],nexp)");
  w.factory("Gaussian:constraint(b0[0,10],b,sigmab[0.1])");
